...
(function ($) { // hide the namespace
    ...
    function Keypad() {
        ...
        this.addKeyDef('ENTER', 'enter', function (inst) {
            $.keypad._selectValue(inst, '\x0D');
        }, true);
        this.addKeyDef('TAB', 'tab', function (inst) {
            $.keypad._selectValue(inst, '\x09');
        }, true);
    ...
    $.extend(Keypad.prototype, {
        ...
        _attachKeypad: function (target, settings) {
            ...
            if (inline) {
                $(target).append(inst._mainDiv).bind('click.keypad', function () {
                    inst._input.focus();
                });
        ...
        _connectKeypad: function (target, inst) {
            ...
            field.addClass(this.markerClassName)
                [this._get(inst, 'keypadOnly') ? 'attr' : 'removeAttr']('readonly', true).bind('setData.keypad', function (event, key, value) {
        ...
        _destroyKeypad: function (target) {
            ...
            $target.siblings('.' + this._appendClass).remove().end().siblings('.' + this._triggerClass).remove().end().prev('.' + this._inlineEntryClass).remove();
            $target.empty().unbind('focus', this._showKeypad).removeClass(this.markerClassName)
        ...
        _enableKeypad: function (target) {
            ...
            if (nodeName == 'input' || nodeName == 'textarea') {
                ...
                control.siblings('button.' + this._triggerClass).each(function () {
                    this.disabled = false;
        ...
        _updateKeypad: function (inst) {
            ...
            inst._mainDiv.empty().append(this._generateHTML(inst)).find('iframe.' + this._coverClass). // IE6- only
                css({
                    left: -borders[0], top: -borders[1],
                    width: inst._mainDiv.outerWidth(), height: inst._mainDiv.outerHeight()
                });
    ...
    $.fn.keypad = function (options) {
        ...
        if (options == 'isDisabled') {
            return $.keypad['_' + options + 'Keypad'].apply($.keypad, [this[0]].concat(otherArgs));
        }
        return this.each(function () {
            typeof options == 'string' ?
                $.keypad['_' + options + 'Keypad'].apply($.keypad, [this].concat(otherArgs)) :
...
// Add the keypad division and external click check
    $(function () {
        $(document.body).append($.keypad.mainDiv).mousedown($.keypad._checkExternalClick);
    });
