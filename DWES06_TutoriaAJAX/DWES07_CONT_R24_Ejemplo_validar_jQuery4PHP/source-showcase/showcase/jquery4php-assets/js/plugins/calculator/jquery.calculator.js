...
(function ($) { // hide the namespace
    ...
    $.extend(Calculator.prototype, {
        ...
        _attachPlugin: function (target, options) {
            ...
            if (inline) {
                $target.append(keyEntry).append(inst._mainDiv).bind('click.' + plugin.propertyName, function () {
                    keyEntry.focus();
                });
                this._reset(inst, '0');
                this._setValue(inst);
        ...
        _disablePlugin: function (target) {
            ...
            if (nodeName == 'input') {
                ...
                control.find('.' + this._inlineEntryClass + ',button').attr('disabled', 'disabled').end().prepend('<div class="' + this._disableClass + '" style="width: ' +
        ...
        _updateCalculator: function (inst) {
            ...
            inst._mainDiv.html(this._generateHTML(inst)).find('iframe.' + this._coverClass). // IE6- only
                css({
                    left: -borders[0], top: -borders[1],
                    width: inst._mainDiv.outerWidth(), height: inst._mainDiv.outerHeight()
                });
