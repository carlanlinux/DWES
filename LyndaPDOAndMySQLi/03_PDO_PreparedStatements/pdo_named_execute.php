...
if (isset($_GET['search'])) {
    try {
        ...
        $sql = 'SELECT make, yearmade, mileage, price, description
                ...
                ORDER BY price';
        $stmt = $db->prepare($sql);
        //Se crea un array que contenga como clave el :parametroQuery. Mandamos los wildcardCharacters para que nos
        // devuelva la consulta lo que contenga esas letras y no importa el orden.
        //Usando execute sólo se pueden pasar Strings.
        $values = array(':price' => $_GET['price'],
                        ':make' => '%' . $_GET['make'] . '%',
                        ':yearmade' => $_GET['yearmade']);
        //Pasamos el array con el método execute
        $stmt->execute($values);
