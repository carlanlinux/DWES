...
<?php      
    ...
    if(!empty($_POST['agendaList']))//1º incializamos el array de agenda comprobando si hay o no datos en el campo oculto.
    {  
      
         $arrayAgenda=$_POST['agendaList'];
...
?>
      ...
      <?php foreach ($arrayAgenda as $clave => $valor) {
          ?> 
                 ...
                 <input   type='hidden'  name='agendaList[<?= $clave?>]' value='<?= $valor?>' />   
