...
<body>
<!-- Aquí ponemos un if con mensaje para mostrar el mensaje, donde si se envía con exíto y no hay ningún mensaje no se
cargue el formulario. Para eso si hay mensaje pasa al if pero en el else se deja abierto con el HTML entre medias y no hace
nada mostrando sólo el formulario  -->
<?php if (isset($msg)) {
    echo '<div id="formmessage"><p>' . $msg . '</p></div>';

} else { ?>
    <!-- Importante, hay que poner el enctype cuando se vayan a enviar ficheros a esto, si no no funciona.. -->
    <form id="myform" name="theform" class="group" action="<?= $_SERVER['PHP_SELF']; ?>" method="POST"
          enctype="multipart/form-data">
        <span id="formerror" class="error"></span>
        <ol>
            <li>
                <label for="myname">Name *</label>
                <input type="text" name="myname" id="myname" title="Please enter your name" autofocus
                       placeholder="Last, First" value="<?php if (isset($myname)) echo $myname ?>"/>
                <?php if (isset($err_myname)) echo $err_myname; ?>
                <?php if (isset($err_patternmatch)) echo $err_patternmatch; ?>
            </li>
            <li>
                <label for="profilepix">Profile Picture (jpeg file)</label>
                <!-- Creamos un campo oculto para indicar el tamaño máximo del fichero, en bytes. -->
                <input type="hidden" name="MAX_FILE_SIZE" value="10240">
                <input type="file" name="myprofilepix" id="profilepix" accept="image/*">
            </li>
            <li>
            ...
            <li>
                ...
                <?php if (isset($_err_mypasswordConf)) echo $_err_mypasswordConf; ?>
            </li>
            <li>
                ...
                <ol>
                    ...
                    <li>
                        <input type="checkbox" name="favoritemusic[]" value="reggaeton" id="reggaetonitem"
                            <?php if ((isset($favoritemusic)) && in_array("reggaetonitem", $favoritemusic)) echo "checked" ?>/>
            ...
            <li>
                ...
                <select name="reference" id="reference">
                    ...
                    <option value="friend" <?php if ((isset($reference)) && ($reference === "friend")) echo "selected"; ?>
                    >A friend
                    </option>
                    <option value="facebook" <?php if ((isset($reference)) && ($reference === "facebook")) echo "selected"; ?>
                    >Facebook
                    </option>
                    <option value="twitter" <?php if ((isset($reference)) && ($reference === "twitter")) echo " selected"; ?>
                    >Twitter
                    </option>
                </select>
            </li>
            ...
            <li>
                ...
                <textarea name="mycomments"
                          id="mycomments"><?php if (isset($mycomments)) echo "$mycomments" ?></textarea>
            </li>
        </ol>
