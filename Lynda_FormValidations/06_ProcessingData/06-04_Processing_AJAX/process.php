...
if (($_SERVER['REQUEST_METHOD'] == 'POST') && (!empty($_POST['action']))):
	...
	//Comprobamos que exista la variable AjaxRequest
	...
	if (strlen($mypassword) <= 6):
		...
		//Comprobamos que si el formulario viene de Ajax, devolvemos el error por AJAX pintando un script en la página.
  ...
  $formdata = array (
	...
	if (!($formerrors)) :
		...
		if (mail($to, $subject, $message)):
			...
			//Comprobamos que si el formulario viene de Ajax, devolvemos el error por AJAX pintando un script en la página.
			if ( $ajaxrequest ) : echo "<script>$('#myform').before('<div id=\"formmessage\"><p>Thanks for filling ' +
 			'out our form. An email has been sent with your request</p></div>'); $('#myform').hide();</script>"; endif;
		else:
